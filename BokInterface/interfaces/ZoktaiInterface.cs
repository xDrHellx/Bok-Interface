...
namespace BokInterface {
    partial class BokInterface {
        ...
        private void AddZoktaiEventsMenu() {
            ...
            AddZoktaiEventHandler(_enableBlindboxLvl3, "blindbox_lvl_3", 0x8E67);
            ...
            AddZoktaiEventHandler(_enableBlindboxLvl4, "blindbox_lvl_4", 0x8FAA);
            ...
            AddZoktaiEventHandler(_enableBlindboxLvl5ValentineDay, "blindbox_lvl_5_valentine_day", 0x90E0);
            ...
            AddZoktaiEventHandler(_enableStarPiece, "star_piece", 0x8FD6);
        }

        /// <summary>Add a Click event on a menu item for a Zoktai / Bok 2 in-game event</summary>
        /// <param name="menuItem">Menu item</param>
        /// <param name="memKey">Related key within the _zoktaiAddresses.Events dictionnary</param>
        /// <param name="onCheckedValue">Value to set on the key when the menu item is checked</param>
        private void AddZoktaiEventHandler(ToolStripMenuItem menuItem, string memKey, uint onCheckedValue, uint onUncheckedValue = 0x0) {
            if (shorterGameName != "Zoktai" || _zoktaiAddresses.Events.ContainsKey(memKey) == false) {
                return;
            }
            ...
            menuItem.Click += (s, e) => {
                /**
                 * If the in-game event is already enabled, check the menu item & stop
                 * In-game events cannot be disabled except by deleting savefiles or loading savestates
                 */
                if (_zoktaiAddresses.Events[memKey].Value == onCheckedValue) {
                    menuItem.Checked = true;
                    return;
                }

                menuItem.Checked = !menuItem.Checked;
                _zoktaiAddresses.Events[memKey].Value = menuItem.Checked == true ? onCheckedValue : onUncheckedValue;
            };
        }

        /// <summary>Update an event menu item for a Zoktai / Bok 2 in-game event</summary>
        /// <param name="menuItem">Menu item</param>
        /// <param name="memKey">Related key within the _zoktaiAddresses.Events dictionnary</param>
        /// <param name="onCheckedValue">Value used for enabling the in-game event</param>
        private void UpdateZoktaiEvent(ToolStripMenuItem menuItem, string memKey, uint onCheckedValue) {
            if (shorterGameName != "Zoktai" || _zoktaiAddresses.Events.ContainsKey(memKey) == false) {
                return;
            }

            // Check / uncheck the menu item based on if the in-game event is enabled or not
            menuItem.Checked = _zoktaiAddresses.Events[memKey].Value == onCheckedValue;
        }
