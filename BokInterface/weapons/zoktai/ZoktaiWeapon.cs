namespace BokInterface.Weapons {
    ...
    class ZoktaiWeapon(string name, uint value, string type, string icon = "", int durability = 0, int bonus = 0, string spEffect = "", int baseDamage = 1, int level = 1, int row = 1, bool rRank = false, int buyPrice = 0, bool eventWeapon = false, bool adjustToLevel = false) : Weapon(name, value, type, icon, durability, spEffect, baseDamage, level, row, buyPrice, eventWeapon, adjustToLevel) {
